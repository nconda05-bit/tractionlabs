...
async def ask_claude(system_message: str, prompt: str, session_id: str = "traction") -> str:
    ...
    try:
        ...
        # send_message may return a string or an object with .text
...
async def ask_claude_json(system_message: str, prompt: str, session_id: str = "traction"):
    full = (
        f"{system_message}\n\nRespond with ONLY valid JSON. No markdown, no commentary, no code fences."
    )
    raw = await ask_claude(full, prompt, session_id=session_id)
